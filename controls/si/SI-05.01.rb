control 'SI-05(01)' do
  impact 0.5
  title "#{input('si_05_01_odp')} are used to broadcast security alert and advisory information throughout the organization."
  desc <<~DESC
    Broadcast security alert and advisory information throughout the organization using #{input('si_05_01_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('si_05_01_odp')} are used to broadcast security alert and advisory information throughout the organization.
  ...
  tag nist: ['SI-5 (1)']
  tag control: 'SI-5 (1)'
  tag objective: 'SI-05(01)'
  tag rev: 'Rev_5'
  tag family: 'System and Information Integrity'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_si_5_1: true
  tag odp: %w{si_05_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective SI-05(01)' do
