control 'SI-07(09)' do
  impact 0.5
  title "the integrity of the boot process of #{input('si_07_09_odp')} is verified."
  desc <<~DESC
    Verify the integrity of the boot process of the following system components: #{input('si_07_09_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the integrity of the boot process of #{input('si_07_09_odp')} is verified.
  ...
  tag nist: ['SI-7 (9)']
  tag control: 'SI-7 (9)'
  tag objective: 'SI-07(09)'
  tag rev: 'Rev_5'
  tag family: 'System and Information Integrity'
  tag baseline: %w{}
  tag control_si_7_9: true
  tag odp: %w{si_07_09_odp}

  describe 'NIST SP 800-53A Rev 5 objective SI-07(09)' do
