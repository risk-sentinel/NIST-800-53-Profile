control 'SI-21' do
  impact 0.5
  title "the #{input('si_21_odp_01')} is refreshed #{input('si_21_odp_02')} or is generated on demand and deleted when no longer needed."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the #{input('si_21_odp_01')} is refreshed #{input('si_21_odp_02')} or is generated on demand and deleted when no longer needed.
  ...
  tag baseline: %w{}
  tag control_si_21: true
  tag odp: %w{si_21_odp_01 si_21_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective SI-21' do
