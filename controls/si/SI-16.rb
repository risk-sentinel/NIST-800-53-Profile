control 'SI-16' do
  impact 0.5
  title "#{input('si_16_odp')} are implemented to protect the system memory from unauthorized code execution."
  desc <<~DESC
    Implement the following controls to protect the system memory from unauthorized code execution: #{input('si_16_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('si_16_odp')} are implemented to protect the system memory from unauthorized code execution.
  ...
  tag control_si_16: true
  tag odp: %w{si_16_odp}

  describe 'NIST SP 800-53A Rev 5 objective SI-16' do
