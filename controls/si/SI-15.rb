control 'SI-15' do
  impact 0.5
  title "information output from #{input('si_15_odp')} is validated to ensure that the information is consistent with the expected content."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      information output from #{input('si_15_odp')} is validated to ensure that the information is consistent with the expected content.
  ...
  tag baseline: %w{}
  tag control_si_15: true
  tag odp: %w{si_15_odp}

  describe 'NIST SP 800-53A Rev 5 objective SI-15' do
