control 'SI-03c.01[02]' do
  impact 0.5
  title "malicious code protection mechanisms are configured to perform real-time scans of files from external sources at #{input('si_03_odp_03')} as the files are downloaded, opened, or executed in accordance with organizational policy;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      malicious code protection mechanisms are configured to perform real-time scans of files from external sources at #{input('si_03_odp_03')} as the files are downloaded, opened, or executed in accordance with organizational policy;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SI-03c.01[02]' do
