control 'SI-07(07)' do
  impact 0.5
  title "the detection of #{input('si_07_07_odp')} are incorporated into the organizational incident response capability."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the detection of #{input('si_07_07_odp')} are incorporated into the organizational incident response capability.
  ...
  tag nist: ['SI-7 (7)']
  tag control: 'SI-7 (7)'
  tag objective: 'SI-07(07)'
  tag rev: 'Rev_5'
  tag family: 'System and Information Integrity'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_si_7_7: true
  tag odp: %w{si_07_07_odp}

  describe 'NIST SP 800-53A Rev 5 objective SI-07(07)' do
