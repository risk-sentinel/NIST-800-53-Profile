control 'SI-07(05)' do
  impact 0.5
  title "#{input('si_07_05_odp_01')} are automatically performed when integrity violations are discovered."
  desc <<~DESC
    Automatically #{input('si_07_05_odp_01')} when integrity violations are discovered.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('si_07_05_odp_01')} are automatically performed when integrity violations are discovered.
  ...
  tag nist: ['SI-7 (5)']
  tag control: 'SI-7 (5)'
  tag objective: 'SI-07(05)'
  tag rev: 'Rev_5'
  tag family: 'System and Information Integrity'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_si_7_5: true
  tag odp: %w{si_07_05_odp_01}

  describe 'NIST SP 800-53A Rev 5 objective SI-07(05)' do
