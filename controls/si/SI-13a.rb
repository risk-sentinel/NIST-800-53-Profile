control 'SI-13a.' do
  impact 0.5
  title "mean time to failure (MTTF) is determined for #{input('si_13_odp_01')} in specific environments of operation;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      mean time to failure (MTTF) is determined for #{input('si_13_odp_01')} in specific environments of operation;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SI-13a.' do
