control 'SI-14[01]' do
  impact 0.5
  title "non-persistent #{input('si_14_odp_01')} that are initiated in a known state are implemented;"
  desc <<~DESC
    Implement non-persistent #{input('si_14_odp_01')} that are initiated in a known state and terminated #{input('si_14_odp_02')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      non-persistent #{input('si_14_odp_01')} that are initiated in a known state are implemented;
