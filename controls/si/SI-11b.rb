control 'SI-11b.' do
  impact 0.5
  title "error messages are revealed only to #{input('si_11_odp')}."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      error messages are revealed only to #{input('si_11_odp')}.
  ...
  describe 'NIST SP 800-53A Rev 5 objective SI-11b.' do
