control 'AU-14a.[02]' do
  impact 0.5
  title "the capability for #{input('au_14_odp_01')} to #{input('au_14_odp_02')} the content of a user session under #{input('au_14_odp_03')} is implemented;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the capability for #{input('au_14_odp_01')} to #{input('au_14_odp_02')} the content of a user session under #{input('au_14_odp_03')} is implemented;
  ...
  tag nist: ['AU-14']
  tag control: 'AU-14'
  tag objective: 'AU-14a.[02]'
  tag rev: 'Rev_5'
  tag family: 'Audit and Accountability'
  tag baseline: %w{}
  tag control_au_14: true
  tag odp: %w{au_14_odp_01 au_14_odp_02 au_14_odp_03}

  describe 'NIST SP 800-53A Rev 5 objective AU-14a.[02]' do
