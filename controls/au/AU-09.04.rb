control 'AU-09(04)' do
  impact 0.5
  title "access to management of audit logging functionality is authorized only to #{input('au_09_04_odp')}."
  desc <<~DESC
    Authorize access to management of audit logging functionality to only #{input('au_09_04_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      access to management of audit logging functionality is authorized only to #{input('au_09_04_odp')}.
  ...
  tag nist: ['AU-9 (4)']
  tag control: 'AU-9 (4)'
  tag objective: 'AU-09(04)'
  tag rev: 'Rev_5'
  tag family: 'Audit and Accountability'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_au_9_4: true
  tag odp: %w{au_09_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective AU-09(04)' do
