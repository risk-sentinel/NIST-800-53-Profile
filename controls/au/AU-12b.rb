control 'AU-12b.' do
  impact 0.5
  title "#{input('au_12_odp_02')} is/are allowed to select the event types that are to be logged by specific components of the system;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('au_12_odp_02')} is/are allowed to select the event types that are to be logged by specific components of the system;
  ...
  describe 'NIST SP 800-53A Rev 5 objective AU-12b.' do
