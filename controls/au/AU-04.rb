control 'AU-04' do
  impact 0.5
  title "audit log storage capacity is allocated to accommodate #{input('au_04_odp')}."
  desc <<~DESC
    Allocate audit log storage capacity to accommodate #{input('au_04_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      audit log storage capacity is allocated to accommodate #{input('au_04_odp')}.
  ...
  tag control_au_4: true
  tag odp: %w{au_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective AU-04' do
