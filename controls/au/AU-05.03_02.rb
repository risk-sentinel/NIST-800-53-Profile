control 'AU-05(03)[02]' do
  impact 0.5
  title "network traffic is #{input('au_05_03_odp')} if network traffic volume is above configured thresholds."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      network traffic is #{input('au_05_03_odp')} if network traffic volume is above configured thresholds.
  ...
  tag nist: ['AU-5 (3)']
  tag control: 'AU-5 (3)'
  tag objective: 'AU-05(03)[02]'
  tag rev: 'Rev_5'
  tag family: 'Audit and Accountability'
  tag baseline: %w{}
  tag control_au_5_3: true
  tag odp: %w{au_05_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective AU-05(03)[02]' do
