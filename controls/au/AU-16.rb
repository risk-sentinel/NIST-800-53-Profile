control 'AU-16' do
  impact 0.5
  title "#{input('au_16_odp_01')} for coordinating #{input('au_16_odp_02')} among external organizations when audit information is transmitted across organizational boundaries are employed."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('au_16_odp_01')} for coordinating #{input('au_16_odp_02')} among external organizations when audit information is transmitted across organizational boundaries are employed.
  ...
  tag baseline: %w{}
  tag control_au_16: true
  tag odp: %w{au_16_odp_01 au_16_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective AU-16' do
