control 'AU-09(06)' do
  impact 0.5
  title "read-only access to audit information is authorized to #{input('au_09_06_odp')}."
  desc <<~DESC
    Authorize read-only access to audit information to #{input('au_09_06_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      read-only access to audit information is authorized to #{input('au_09_06_odp')}.
  ...
  tag nist: ['AU-9 (6)']
  tag control: 'AU-9 (6)'
  tag objective: 'AU-09(06)'
  tag rev: 'Rev_5'
  tag family: 'Audit and Accountability'
  tag baseline: %w{}
  tag control_au_9_6: true
  tag odp: %w{au_09_06_odp}

  describe 'NIST SP 800-53A Rev 5 objective AU-09(06)' do
