control 'IR-05(01)[01]' do
  impact 0.5
  title "incidents are tracked using #{input('ir_05_01_odp_01')};"
  desc <<~DESC
    Track incidents and collect and analyze incident information using #{input('ir_5_1_prm_1')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      incidents are tracked using #{input('ir_05_01_odp_01')};
  ...
  tag nist: ['IR-5 (1)']
  tag control: 'IR-5 (1)'
  tag objective: 'IR-05(01)[01]'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_ir_5_1: true
  tag odp: %w{ir_05_01_odp_01 ir_5_1_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective IR-05(01)[01]' do
