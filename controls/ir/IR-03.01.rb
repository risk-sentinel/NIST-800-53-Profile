control 'IR-03(01)' do
  impact 0.5
  title "the incident response capability is tested using #{input('ir_03_01_odp')}."
  desc <<~DESC
    Test the incident response capability using #{input('ir_03_01_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the incident response capability is tested using #{input('ir_03_01_odp')}.
  ...
  tag nist: ['IR-3 (1)']
  tag control: 'IR-3 (1)'
  tag objective: 'IR-03(01)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{}
  tag control_ir_3_1: true
  tag odp: %w{ir_03_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-03(01)' do
