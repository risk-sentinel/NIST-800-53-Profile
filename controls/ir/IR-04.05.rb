control 'IR-04(05)' do
  impact 0.5
  title "a configurable capability is implemented to automatically disable the system if #{input('ir_04_05_odp')} are detected."
  desc <<~DESC
    Implement a configurable capability to automatically disable the system if #{input('ir_04_05_odp')} are detected.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      a configurable capability is implemented to automatically disable the system if #{input('ir_04_05_odp')} are detected.
  ...
  tag nist: ['IR-4 (5)']
  tag control: 'IR-4 (5)'
  tag objective: 'IR-04(05)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{}
  tag control_ir_4_5: true
  tag odp: %w{ir_04_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-04(05)' do
