control 'IR-06(01)' do
  impact 0.5
  title "incidents are reported using #{input('ir_06_01_odp')}."
  desc <<~DESC
    Report incidents using #{input('ir_06_01_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      incidents are reported using #{input('ir_06_01_odp')}.
  ...
  tag nist: ['IR-6 (1)']
  tag control: 'IR-6 (1)'
  tag objective: 'IR-06(01)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ir_6_1: true
  tag odp: %w{ir_06_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-06(01)' do
