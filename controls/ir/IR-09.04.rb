control 'IR-09(04)' do
  impact 0.5
  title "#{input('ir_09_04_odp')} are employed for personnel exposed to information not within assigned access authorizations."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('ir_09_04_odp')} are employed for personnel exposed to information not within assigned access authorizations.
  ...
  tag nist: ['IR-9 (4)']
  tag control: 'IR-9 (4)'
  tag objective: 'IR-09(04)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{}
  tag control_ir_9_4: true
  tag odp: %w{ir_09_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-09(04)' do
