control 'IR-09(02)' do
  impact 0.5
  title "information spillage response training is provided #{input('ir_09_02_odp')}."
  desc <<~DESC
    Provide information spillage response training #{input('ir_09_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      information spillage response training is provided #{input('ir_09_02_odp')}.
  ...
  tag nist: ['IR-9 (2)']
  tag control: 'IR-9 (2)'
  tag objective: 'IR-09(02)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{}
  tag control_ir_9_2: true
  tag odp: %w{ir_09_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-09(02)' do
