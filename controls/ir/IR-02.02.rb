control 'IR-02(02)' do
  impact 0.5
  title "an incident response training environment is provided using #{input('ir_02_02_odp')}."
  desc <<~DESC
    Provide an incident response training environment using #{input('ir_02_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      an incident response training environment is provided using #{input('ir_02_02_odp')}.
  ...
  tag nist: ['IR-2 (2)']
  tag control: 'IR-2 (2)'
  tag objective: 'IR-02(02)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_ir_2_2: true
  tag odp: %w{ir_02_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-02(02)' do
