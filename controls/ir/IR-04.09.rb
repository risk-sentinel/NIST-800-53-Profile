control 'IR-04(09)' do
  impact 0.5
  title "#{input('ir_04_09_odp')} are employed to respond to incidents."
  desc <<~DESC
    Employ #{input('ir_04_09_odp')} to respond to incidents.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ir_04_09_odp')} are employed to respond to incidents.
  ...
  tag nist: ['IR-4 (9)']
  tag control: 'IR-4 (9)'
  tag objective: 'IR-04(09)'
  tag rev: 'Rev_5'
  tag family: 'Incident Response'
  tag baseline: %w{}
  tag control_ir_4_9: true
  tag odp: %w{ir_04_09_odp}

  describe 'NIST SP 800-53A Rev 5 objective IR-04(09)' do
