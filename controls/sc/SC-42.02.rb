control 'SC-42(02)' do
  impact 0.5
  title "#{input('sc_42_02_odp')} are employed so that data or information collected by #{input('sc_42_01_odp')} is only used for authorized purposes."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_42_02_odp')} are employed so that data or information collected by #{input('sc_42_01_odp')} is only used for authorized purposes.
  ...
  tag nist: ['SC-42 (2)']
  tag control: 'SC-42 (2)'
  tag objective: 'SC-42(02)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_42_2: true
  tag odp: %w{sc_42_01_odp sc_42_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-42(02)' do
