control 'SC-34a.' do
  impact 0.5
  title "the operating environment for #{input('sc_34_odp_01')} is loaded and executed from hardware-enforced, read-only media;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the operating environment for #{input('sc_34_odp_01')} is loaded and executed from hardware-enforced, read-only media;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-34a.' do
