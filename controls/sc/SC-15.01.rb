control 'SC-15(01)' do
  impact 0.5
  title "the #{input('sc_15_01_odp')} disconnect of collaborative computing devices is/are provided in a manner that supports ease of use."
  desc <<~DESC
    Provide #{input('sc_15_01_odp')} disconnect of collaborative computing devices in a manner that supports ease of use.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the #{input('sc_15_01_odp')} disconnect of collaborative computing devices is/are provided in a manner that supports ease of use.
  ...
  tag nist: ['SC-15 (1)']
  tag control: 'SC-15 (1)'
  tag objective: 'SC-15(01)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_15_1: true
  tag odp: %w{sc_15_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-15(01)' do
