control 'SC-44' do
  impact 0.5
  title "a detonation chamber capability is employed within the #{input('sc_44_odp')}."
  desc <<~DESC
    Employ a detonation chamber capability within #{input('sc_44_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      a detonation chamber capability is employed within the #{input('sc_44_odp')}.
  ...
  tag baseline: %w{}
  tag control_sc_44: true
  tag odp: %w{sc_44_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-44' do
