control 'SC-28(02)[02]' do
  impact 0.5
  title "#{input('sc_28_02_odp')} is stored offline in a secure location."
  desc <<~DESC
    Remove the following information from online storage and store offline in a secure location: #{input('sc_28_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_28_02_odp')} is stored offline in a secure location.
  ...
  tag nist: ['SC-28 (2)']
  tag control: 'SC-28 (2)'
  tag objective: 'SC-28(02)[02]'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_28_2: true
  tag odp: %w{sc_28_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-28(02)[02]' do
