control 'SC-40(04)' do
  impact 0.5
  title "cryptographic mechanisms are implemented to prevent the identification of #{input('sc_40_04_odp')} by using the transmitter signal parameters."
  desc <<~DESC
    Implement cryptographic mechanisms to prevent the identification of #{input('sc_40_04_odp')} by using the transmitter signal parameters.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      cryptographic mechanisms are implemented to prevent the identification of #{input('sc_40_04_odp')} by using the transmitter signal parameters.
    ...
    Assessment methods and objects:
      ...
      TEST: Cryptographic mechanisms preventing the identification of wireless transmitters
  CHECK
  desc 'fix', <<~FIX
  ...
  tag nist: ['SC-40 (4)']
  tag control: 'SC-40 (4)'
  tag objective: 'SC-40(04)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_40_4: true
  tag odp: %w{sc_40_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-40(04)' do
