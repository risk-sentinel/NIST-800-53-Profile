control 'SC-16[01]' do
  impact 0.5
  title "#{input('sc_16_odp_01')} are associated with information exchanged between systems;"
  desc <<~DESC
    Associate #{input('sc_16_prm_1')} with information exchanged between systems and between system components.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_16_odp_01')} are associated with information exchanged between systems;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-16[01]' do
