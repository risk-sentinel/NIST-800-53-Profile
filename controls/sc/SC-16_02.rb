control 'SC-16[02]' do
  impact 0.5
  title "#{input('sc_16_odp_01')} are associated with information exchanged between system components;"
  desc <<~DESC
    Associate #{input('sc_16_prm_1')} with information exchanged between systems and between system components.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_16_odp_01')} are associated with information exchanged between system components;
  ...
  tag nist: ['SC-16']
  tag control: 'SC-16'
  tag objective: 'SC-16[02]'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_16: true
  tag odp: %w{sc_16_odp_01 sc_16_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective SC-16[02]' do
