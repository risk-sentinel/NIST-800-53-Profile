control 'SC-49' do
  impact 0.5
  title "hardware-enforced separation and policy enforcement mechanisms are implemented between #{input('sc_49_odp')}."
  desc <<~DESC
    Implement hardware-enforced separation and policy enforcement mechanisms between #{input('sc_49_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      hardware-enforced separation and policy enforcement mechanisms are implemented between #{input('sc_49_odp')}.
    ...
    Assessment methods and objects:
      ...
      TEST: Mechanisms supporting and/or implementing hardware-enforced security domain separation and policy enforcement
  CHECK
  desc 'fix', <<~FIX
  ...
  tag baseline: %w{}
  tag control_sc_49: true
  tag odp: %w{sc_49_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-49' do
