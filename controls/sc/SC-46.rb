control 'SC-46' do
  impact 0.5
  title "a policy enforcement mechanism is #{input('sc_46_odp')} implemented between the physical and/or network interfaces for the connecting security domains."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      a policy enforcement mechanism is #{input('sc_46_odp')} implemented between the physical and/or network interfaces for the connecting security domains.
  ...
  tag baseline: %w{}
  tag control_sc_46: true
  tag odp: %w{sc_46_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-46' do
