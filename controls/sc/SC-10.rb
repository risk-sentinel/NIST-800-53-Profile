control 'SC-10' do
  impact 0.5
  title "the network connection associated with a communication session is terminated at the end of the session or after #{input('sc_10_odp')} of inactivity."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the network connection associated with a communication session is terminated at the end of the session or after #{input('sc_10_odp')} of inactivity.
  ...
  tag control_sc_10: true
  tag odp: %w{sc_10_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-10' do
