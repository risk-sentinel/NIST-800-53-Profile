control 'SC-34(01)' do
  impact 0.5
  title "#{input('sc_34_01_odp')} are employed with no writeable storage that is persistent across component restart or power on/off."
  desc <<~DESC
    Employ #{input('sc_34_01_odp')} with no writeable storage that is persistent across component restart or power on/off.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_34_01_odp')} are employed with no writeable storage that is persistent across component restart or power on/off.
  ...
  tag nist: ['SC-34 (1)']
  tag control: 'SC-34 (1)'
  tag objective: 'SC-34(01)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_34_1: true
  tag odp: %w{sc_34_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-34(01)' do
