control 'SC-15(04)' do
  impact 0.5
  title "an explicit indication of current participants in #{input('sc_15_04_odp')} is provided."
  desc <<~DESC
    Provide an explicit indication of current participants in #{input('sc_15_04_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      an explicit indication of current participants in #{input('sc_15_04_odp')} is provided.
  ...
  tag nist: ['SC-15 (4)']
  tag control: 'SC-15 (4)'
  tag objective: 'SC-15(04)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_15_4: true
  tag odp: %w{sc_15_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-15(04)' do
