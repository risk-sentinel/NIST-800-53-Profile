control 'SC-12[02]' do
  impact 0.5
  title "cryptographic keys are managed when cryptography is employed within the system in accordance with #{input('sc_12_odp')}."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      cryptographic keys are managed when cryptography is employed within the system in accordance with #{input('sc_12_odp')}.
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-12[02]' do
