control 'SC-50' do
  impact 0.5
  title "software-enforced separation and policy enforcement mechanisms are implemented between #{input('sc_50_odp')}."
  desc <<~DESC
    Implement software-enforced separation and policy enforcement mechanisms between #{input('sc_50_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      software-enforced separation and policy enforcement mechanisms are implemented between #{input('sc_50_odp')}.
  ...
  tag baseline: %w{}
  tag control_sc_50: true
  tag odp: %w{sc_50_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-50' do
