control 'SC-07(19)[01]' do
  impact 0.5
  title "inbound communications traffic is blocked between #{input('sc_07_19_odp')} that are independently configured by end users and external service providers;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      inbound communications traffic is blocked between #{input('sc_07_19_odp')} that are independently configured by end users and external service providers;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-07(19)[01]' do
