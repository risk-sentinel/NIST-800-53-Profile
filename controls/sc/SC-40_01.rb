control 'SC-40[01]' do
  impact 0.5
  title "external #{input('sc_40_odp_01')} are protected from #{input('sc_40_odp_02')}."
  desc <<~DESC
    Protect external and internal #{input('sc_40_prm_1')} from the following signal parameter attacks: #{input('sc_40_prm_2')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      external #{input('sc_40_odp_01')} are protected from #{input('sc_40_odp_02')}.
  ...
  tag baseline: %w{}
  tag control_sc_40: true
  tag odp: %w{sc_40_odp_01 sc_40_odp_02 sc_40_prm_1 sc_40_prm_2}
