control 'SC-39(02)' do
  impact 0.5
  title "a separate execution domain is maintained for each thread in #{input('sc_39_02_odp')}."
  desc <<~DESC
    Maintain a separate execution domain for each thread in #{input('sc_39_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      a separate execution domain is maintained for each thread in #{input('sc_39_02_odp')}.
  ...
  tag nist: ['SC-39 (2)']
  tag control: 'SC-39 (2)'
  tag objective: 'SC-39(02)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_39_2: true
  tag odp: %w{sc_39_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-39(02)' do
