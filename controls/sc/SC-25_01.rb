control 'SC-25[01]' do
  impact 0.5
  title "minimal functionality for #{input('sc_25_odp')} is employed;"
  desc <<~DESC
    Employ minimal functionality and information storage on the following system components: #{input('sc_25_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      minimal functionality for #{input('sc_25_odp')} is employed;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-25[01]' do
