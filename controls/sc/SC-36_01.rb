control 'SC-36[01]' do
  impact 0.5
  title "#{input('sc_36_odp_01')} are distributed across #{input('sc_36_odp_02')};"
  desc <<~DESC
    Distribute the following processing and storage components across multiple #{input('sc_36_prm_1')}: #{input('sc_36_prm_2')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_36_odp_01')} are distributed across #{input('sc_36_odp_02')};
  ...
  tag nist: ['SC-36']
  tag control: 'SC-36'
  tag objective: 'SC-36[01]'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_36: true
  tag odp: %w{sc_36_odp_01 sc_36_odp_02 sc_36_prm_1 sc_36_prm_2}

  describe 'NIST SP 800-53A Rev 5 objective SC-36[01]' do
