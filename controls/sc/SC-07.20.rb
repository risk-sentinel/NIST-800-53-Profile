control 'SC-07(20)' do
  impact 0.5
  title "the capability to dynamically isolate #{input('sc_07_20_odp')} from other system components is provided."
  desc <<~DESC
    Provide the capability to dynamically isolate #{input('sc_07_20_odp')} from other system components.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the capability to dynamically isolate #{input('sc_07_20_odp')} from other system components is provided.
  ...
  tag nist: ['SC-7 (20)']
  tag control: 'SC-7 (20)'
  tag objective: 'SC-07(20)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_7_20: true
  tag odp: %w{sc_07_20_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-07(20)' do
