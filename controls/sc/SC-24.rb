control 'SC-24' do
  impact 0.5
  title "#{input('sc_24_odp_01')} fail to a #{input('sc_24_odp_02')} while preserving #{input('sc_24_odp_03')} in failure."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_24_odp_01')} fail to a #{input('sc_24_odp_02')} while preserving #{input('sc_24_odp_03')} in failure.
  ...
  tag control_sc_24: true
  tag odp: %w{sc_24_odp_01 sc_24_odp_02 sc_24_odp_03}

  describe 'NIST SP 800-53A Rev 5 objective SC-24' do
