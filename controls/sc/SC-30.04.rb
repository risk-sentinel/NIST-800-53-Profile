control 'SC-30(04)' do
  impact 0.5
  title "realistic but misleading information about the security state or posture of #{input('sc_30_04_odp')} is employed."
  desc <<~DESC
    Employ realistic, but misleading information in #{input('sc_30_04_odp')} about its security state or posture.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      realistic but misleading information about the security state or posture of #{input('sc_30_04_odp')} is employed.
  ...
  tag nist: ['SC-30 (4)']
  tag control: 'SC-30 (4)'
  tag objective: 'SC-30(04)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_30_4: true
  tag odp: %w{sc_30_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-30(04)' do
