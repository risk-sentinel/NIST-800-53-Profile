control 'SC-16[03]' do
  impact 0.5
  title "#{input('sc_16_odp_02')} are associated with information exchanged between systems;"
  desc <<~DESC
    Associate #{input('sc_16_prm_1')} with information exchanged between systems and between system components.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_16_odp_02')} are associated with information exchanged between systems;
