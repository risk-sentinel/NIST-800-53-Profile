control 'SC-38' do
  impact 0.5
  title "#{input('sc_38_odp')} are employed to protect key organizational information throughout the system development life cycle."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_38_odp')} are employed to protect key organizational information throughout the system development life cycle.
  ...
  tag baseline: %w{}
  tag control_sc_38: true
  tag odp: %w{sc_38_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-38' do
