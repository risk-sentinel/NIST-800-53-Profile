control 'SC-28(02)[01]' do
  impact 0.5
  title "#{input('sc_28_02_odp')} is removed from online storage;"
  desc <<~DESC
    Remove the following information from online storage and store offline in a secure location: #{input('sc_28_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sc_28_02_odp')} is removed from online storage;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-28(02)[01]' do
