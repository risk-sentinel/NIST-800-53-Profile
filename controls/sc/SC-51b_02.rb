control 'SC-51b.[02]' do
  impact 0.5
  title "specific procedures are implemented for #{input('sc_51_odp_02')} to re-enable the write-protect prior to returning to operational mode."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      specific procedures are implemented for #{input('sc_51_odp_02')} to re-enable the write-protect prior to returning to operational mode.
  ...
  tag nist: ['SC-51']
  tag control: 'SC-51'
  tag objective: 'SC-51b.[02]'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{}
  tag control_sc_51: true
  tag odp: %w{sc_51_odp_01 sc_51_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective SC-51b.[02]' do
