control 'SC-12(03)[01]' do
  impact 0.5
  title "asymmetric cryptographic keys are produced using #{input('sc_12_03_odp')};"
  desc <<~DESC
    Produce, control, and distribute asymmetric cryptographic keys using #{input('sc_12_03_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      asymmetric cryptographic keys are produced using #{input('sc_12_03_odp')};
  ...
  describe 'NIST SP 800-53A Rev 5 objective SC-12(03)[01]' do
