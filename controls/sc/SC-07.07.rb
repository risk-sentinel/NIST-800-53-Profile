control 'SC-07(07)' do
  impact 0.5
  title "split tunneling is prevented for remote devices connecting to organizational systems unless the split tunnel is securely provisioned using #{input('sc_07_07_odp')}."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      split tunneling is prevented for remote devices connecting to organizational systems unless the split tunnel is securely provisioned using #{input('sc_07_07_odp')}.
  ...
  tag nist: ['SC-7 (7)']
  tag control: 'SC-7 (7)'
  tag objective: 'SC-07(07)'
  tag rev: 'Rev_5'
  tag family: 'System and Communications Protection'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_sc_7_7: true
  tag odp: %w{sc_07_07_odp}

  describe 'NIST SP 800-53A Rev 5 objective SC-07(07)' do
