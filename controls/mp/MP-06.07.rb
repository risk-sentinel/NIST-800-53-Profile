control 'MP-06(07)' do
  impact 0.5
  title "dual authorization for sanitization of #{input('mp_06_07_odp')} is enforced."
  desc <<~DESC
    Enforce dual authorization for the sanitization of #{input('mp_06_07_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      dual authorization for sanitization of #{input('mp_06_07_odp')} is enforced.
  ...
  tag nist: ['MP-6 (7)']
  tag control: 'MP-6 (7)'
  tag objective: 'MP-06(07)'
  tag rev: 'Rev_5'
  tag family: 'Media Protection'
  tag baseline: %w{}
  tag control_mp_6_7: true
  tag odp: %w{mp_06_07_odp}

  describe 'NIST SP 800-53A Rev 5 objective MP-06(07)' do
