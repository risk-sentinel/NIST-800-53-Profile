control 'MP-06(02)[01]' do
  impact 0.5
  title "sanitization equipment is tested #{input('mp_06_02_odp_01')} to ensure that the intended sanitization is being achieved;"
  desc <<~DESC
    Test sanitization equipment and procedures #{input('mp_6_2_prm_1')} to ensure that the intended sanitization is being achieved.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      sanitization equipment is tested #{input('mp_06_02_odp_01')} to ensure that the intended sanitization is being achieved;
  ...
  tag nist: ['MP-6 (2)']
  tag control: 'MP-6 (2)'
  tag objective: 'MP-06(02)[01]'
  tag rev: 'Rev_5'
  tag family: 'Media Protection'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_mp_6_2: true
  tag odp: %w{mp_06_02_odp_01 mp_6_2_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective MP-06(02)[01]' do
