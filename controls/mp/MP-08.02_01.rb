control 'MP-08(02)[01]' do
  impact 0.5
  title "downgrading equipment is tested #{input('mp_08_02_odp_01')} to ensure that downgrading actions are being achieved;"
  desc <<~DESC
    Test downgrading equipment and procedures #{input('mp_8_2_prm_1')} to ensure that downgrading actions are being achieved.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      downgrading equipment is tested #{input('mp_08_02_odp_01')} to ensure that downgrading actions are being achieved;
  ...
  tag nist: ['MP-8 (2)']
  tag control: 'MP-8 (2)'
  tag objective: 'MP-08(02)[01]'
  tag rev: 'Rev_5'
  tag family: 'Media Protection'
  tag baseline: %w{}
  tag control_mp_8_2: true
  tag odp: %w{mp_08_02_odp_01 mp_8_2_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective MP-08(02)[01]' do
