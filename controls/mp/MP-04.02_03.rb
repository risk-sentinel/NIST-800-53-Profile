control 'MP-04(02)[03]' do
  impact 0.5
  title "access granted to media storage areas is logged using #{input('mp_04_02_odp_03')}."
  desc <<~DESC
    Restrict access to media storage areas and log access attempts and access granted using #{input('mp_4_2_prm_1')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      access granted to media storage areas is logged using #{input('mp_04_02_odp_03')}.
  ...
  tag nist: ['MP-4 (2)']
  tag control: 'MP-4 (2)'
  tag objective: 'MP-04(02)[03]'
  tag rev: 'Rev_5'
  tag family: 'Media Protection'
  tag baseline: %w{}
  tag control_mp_4_2: true
  tag odp: %w{mp_04_02_odp_03 mp_4_2_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective MP-04(02)[03]' do
