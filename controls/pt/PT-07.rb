control 'PT-07' do
  impact 0.5
  title "#{input('pt_07_odp')} are applied for specific categories of personally identifiable information."
  desc <<~DESC
    Apply #{input('pt_07_odp')} for specific categories of personally identifiable information.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('pt_07_odp')} are applied for specific categories of personally identifiable information.
  ...
  tag control_pt_7: true
  tag odp: %w{pt_07_odp}

  describe 'NIST SP 800-53A Rev 5 objective PT-07' do
