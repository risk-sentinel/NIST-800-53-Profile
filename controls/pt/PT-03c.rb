control 'PT-03c.' do
  impact 0.5
  title "the #{input('pt_03_odp_02')} of personally identifiable information are restricted to only that which is compatible with the identified purpose(s);"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the #{input('pt_03_odp_02')} of personally identifiable information are restricted to only that which is compatible with the identified purpose(s);
  ...
  describe 'NIST SP 800-53A Rev 5 objective PT-03c.' do
