control 'CA-07(06)' do
  impact 0.5
  title "#{input('ca_07_06_odp')} are used to ensure the accuracy, currency, and availability of monitoring results for the system."
  desc <<~DESC
    Ensure the accuracy, currency, and availability of monitoring results for the system using #{input('ca_07_06_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ca_07_06_odp')} are used to ensure the accuracy, currency, and availability of monitoring results for the system.
  ...
  tag nist: ['CA-7 (6)']
  tag control: 'CA-7 (6)'
  tag objective: 'CA-07(06)'
  tag rev: 'Rev_5'
  tag family: 'Assessment, Authorization, and Monitoring'
  tag baseline: %w{}
  tag control_ca_7_6: true
  tag odp: %w{ca_07_06_odp}

  describe 'NIST SP 800-53A Rev 5 objective CA-07(06)' do
