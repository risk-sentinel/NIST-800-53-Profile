control 'PM-32' do
  impact 0.5
  title "#{input('pm_32_odp')} supporting mission-essential services or functions are analyzed to ensure that the information resources are being used in a manner that is consistent with their intended purpose."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('pm_32_odp')} supporting mission-essential services or functions are analyzed to ensure that the information resources are being used in a manner that is consistent with their intended purpose.
  ...
  tag baseline: %w{}
  tag control_pm_32: true
  tag odp: %w{pm_32_odp}

  describe 'NIST SP 800-53A Rev 5 objective PM-32' do
