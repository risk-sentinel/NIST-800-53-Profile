control 'PM-05[02]' do
  impact 0.5
  title "the inventory of organizational systems is updated #{input('pm_05_odp')}."
  desc <<~DESC
    Develop and update #{input('pm_05_odp')} an inventory of organizational systems.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the inventory of organizational systems is updated #{input('pm_05_odp')}.
  ...
  tag nist: ['PM-5']
  tag control: 'PM-5'
  tag objective: 'PM-05[02]'
  tag rev: 'Rev_5'
  tag family: 'Program Management'
  tag baseline: %w{}
  tag control_pm_5: true
  tag odp: %w{pm_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective PM-05[02]' do
