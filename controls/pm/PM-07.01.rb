control 'PM-07(01)' do
  impact 0.5
  title "#{input('pm_07_01_odp')} are offloaded to other systems, system components, or an external provider."
  desc <<~DESC
    Offload #{input('pm_07_01_odp')} to other systems, system components, or an external provider.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('pm_07_01_odp')} are offloaded to other systems, system components, or an external provider.
  ...
  tag nist: ['PM-7 (1)']
  tag control: 'PM-7 (1)'
  tag objective: 'PM-07(01)'
  tag rev: 'Rev_5'
  tag family: 'Program Management'
  tag baseline: %w{}
  tag control_pm_7_1: true
  tag odp: %w{pm_07_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective PM-07(01)' do
