control 'IA-04(05)' do
  impact 0.5
  title "individual identifiers are dynamically managed in accordance with #{input('ia_04_05_odp')}."
  desc <<~DESC
    Manage individual identifiers dynamically in accordance with #{input('ia_04_05_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      individual identifiers are dynamically managed in accordance with #{input('ia_04_05_odp')}.
  ...
  tag nist: ['IA-4 (5)']
  tag control: 'IA-4 (5)'
  tag objective: 'IA-04(05)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{}
  tag control_ia_4_5: true
  tag odp: %w{ia_04_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-04(05)' do
