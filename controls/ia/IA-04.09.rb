control 'IA-04(09)' do
  impact 0.5
  title "the attributes for each uniquely identified individual, device, or service are maintained in #{input('ia_04_09_odp')}."
  desc <<~DESC
    Maintain the attributes for each uniquely identified individual, device, or service in #{input('ia_04_09_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the attributes for each uniquely identified individual, device, or service are maintained in #{input('ia_04_09_odp')}.
  ...
  tag nist: ['IA-4 (9)']
  tag control: 'IA-4 (9)'
  tag objective: 'IA-04(09)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{}
  tag control_ia_4_9: true
  tag odp: %w{ia_04_09_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-04(09)' do
