control 'IA-13[02]' do
  impact 0.5
  title "identity providers are employed to manage user, device, and non-person entity (NPE) identities, attributes and access rights supporting authorization decisions in accordance with #{input('ia_13_odp_02')} using #{input('ia_13_odp_02')};"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      identity providers are employed to manage user, device, and non-person entity (NPE) identities, attributes and access rights supporting authorization decisions in accordance with #{input('ia_13_odp_02')} using #{input('ia_13_odp_02')};
  ...
  describe 'NIST SP 800-53A Rev 5 objective IA-13[02]' do
