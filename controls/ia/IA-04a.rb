control 'IA-04a.' do
  impact 0.5
  title "system identifiers are managed by receiving authorization from #{input('ia_04_odp_01')} to assign to an individual, group, role, or device identifier;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      system identifiers are managed by receiving authorization from #{input('ia_04_odp_01')} to assign to an individual, group, role, or device identifier;
  ...
  tag control_ia_4: true
  tag odp: %w{ia_04_odp_01 ia_04_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective IA-04a.' do
