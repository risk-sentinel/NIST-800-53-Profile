control 'IA-05(08)' do
  impact 0.5
  title "#{input('ia_05_08_odp')} are implemented to manage the risk of compromise due to individuals having accounts on multiple systems."
  desc <<~DESC
    Implement #{input('ia_05_08_odp')} to manage the risk of compromise due to individuals having accounts on multiple systems.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ia_05_08_odp')} are implemented to manage the risk of compromise due to individuals having accounts on multiple systems.
  ...
  tag nist: ['IA-5 (8)']
  tag control: 'IA-5 (8)'
  tag objective: 'IA-05(08)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{}
  tag control_ia_5_8: true
  tag odp: %w{ia_05_08_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-05(08)' do
