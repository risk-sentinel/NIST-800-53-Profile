control 'IA-04(04)' do
  impact 0.5
  title "individual identifiers are managed by uniquely identifying each individual as #{input('ia_04_04_odp')}."
  desc <<~DESC
    Manage individual identifiers by uniquely identifying each individual as #{input('ia_04_04_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      individual identifiers are managed by uniquely identifying each individual as #{input('ia_04_04_odp')}.
  ...
  tag nist: ['IA-4 (4)']
  tag control: 'IA-4 (4)'
  tag objective: 'IA-04(04)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ia_4_4: true
  tag odp: %w{ia_04_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-04(04)' do
