control 'IA-11' do
  impact 0.5
  title "users are required to re-authenticate when #{input('ia_11_odp')}."
  desc <<~DESC
    Require users to re-authenticate when #{input('ia_11_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      users are required to re-authenticate when #{input('ia_11_odp')}.
  ...
  tag control_ia_11: true
  tag odp: %w{ia_11_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-11' do
