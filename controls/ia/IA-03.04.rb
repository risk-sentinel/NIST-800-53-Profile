control 'IA-03(04)' do
  impact 0.5
  title "device identification and authentication are handled based on attestation by #{input('ia_03_04_odp')}."
  desc <<~DESC
    Handle device identification and authentication based on attestation by #{input('ia_03_04_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      device identification and authentication are handled based on attestation by #{input('ia_03_04_odp')}.
  ...
  tag nist: ['IA-3 (4)']
  tag control: 'IA-3 (4)'
  tag objective: 'IA-03(04)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{}
  tag control_ia_3_4: true
  tag odp: %w{ia_03_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-03(04)' do
