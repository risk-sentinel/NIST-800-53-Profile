control 'IA-08(05)[01]' do
  impact 0.5
  title "federated or PKI credentials that meet #{input('ia_08_05_odp')} are accepted;"
  desc <<~DESC
    Accept and verify federated or PKI credentials that meet #{input('ia_08_05_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      federated or PKI credentials that meet #{input('ia_08_05_odp')} are accepted;
  ...
  tag nist: ['IA-8 (5)']
  tag control: 'IA-8 (5)'
  tag objective: 'IA-08(05)[01]'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{}
  tag control_ia_8_5: true
  tag odp: %w{ia_08_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-08(05)[01]' do
