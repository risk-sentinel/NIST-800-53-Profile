control 'IA-12(03)' do
  impact 0.5
  title "the presented identity evidence is validated and verified through #{input('ia_12_03_odp')}."
  desc <<~DESC
    Require that the presented identity evidence be validated and verified through #{input('ia_12_03_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the presented identity evidence is validated and verified through #{input('ia_12_03_odp')}.
  ...
  tag nist: ['IA-12 (3)']
  tag control: 'IA-12 (3)'
  tag objective: 'IA-12(03)'
  tag rev: 'Rev_5'
  tag family: 'Identification and Authentication'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ia_12_3: true
  tag odp: %w{ia_12_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-12(03)' do
