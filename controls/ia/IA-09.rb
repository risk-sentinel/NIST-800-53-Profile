control 'IA-09' do
  impact 0.5
  title "#{input('ia_09_odp')} are uniquely identified and authenticated before establishing communications with devices, users, or other services or applications."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('ia_09_odp')} are uniquely identified and authenticated before establishing communications with devices, users, or other services or applications.
  ...
  tag baseline: %w{}
  tag control_ia_9: true
  tag odp: %w{ia_09_odp}

  describe 'NIST SP 800-53A Rev 5 objective IA-09' do
