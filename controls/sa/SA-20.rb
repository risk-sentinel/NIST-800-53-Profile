control 'SA-20' do
  impact 0.5
  title "#{input('sa_20_odp')} are reimplemented or custom-developed."
  desc <<~DESC
    Reimplement or custom develop the following critical system components: #{input('sa_20_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('sa_20_odp')} are reimplemented or custom-developed.
  ...
  tag baseline: %w{}
  tag control_sa_20: true
  tag odp: %w{sa_20_odp}

  describe 'NIST SP 800-53A Rev 5 objective SA-20' do
