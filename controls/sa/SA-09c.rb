control 'SA-09c.' do
  impact 0.5
  title "#{input('sa_09_odp_02')} are employed to monitor control compliance by external service providers on an ongoing basis."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sa_09_odp_02')} are employed to monitor control compliance by external service providers on an ongoing basis.
  ...
  describe 'NIST SP 800-53A Rev 5 objective SA-09c.' do
