control 'SA-23' do
  impact 0.5
  title "#{input('sa_23_odp_01')} is employed on #{input('sa_23_odp_02')} supporting essential services or functions to increase the trustworthiness in those systems or components."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sa_23_odp_01')} is employed on #{input('sa_23_odp_02')} supporting essential services or functions to increase the trustworthiness in those systems or components.
  ...
  tag baseline: %w{}
  tag control_sa_23: true
  tag odp: %w{sa_23_odp_01 sa_23_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective SA-23' do
