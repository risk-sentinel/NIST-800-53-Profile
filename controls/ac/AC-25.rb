control 'AC-25' do
  impact 0.5
  title "a reference monitor is implemented for #{input('ac_25_odp')} that is tamper-proof, always invoked, and small enough to be subject to analysis and testing, the completeness of which can be assured."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      a reference monitor is implemented for #{input('ac_25_odp')} that is tamper-proof, always invoked, and small enough to be subject to analysis and testing, the completeness of which can be assured.
  ...
  tag baseline: %w{}
  tag control_ac_25: true
  tag odp: %w{ac_25_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-25' do
