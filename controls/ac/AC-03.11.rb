control 'AC-03(11)' do
  impact 0.5
  title "access to data repositories containing #{input('ac_03_11_odp')} is restricted."
  desc <<~DESC
    Restrict access to data repositories containing #{input('ac_03_11_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      access to data repositories containing #{input('ac_03_11_odp')} is restricted.
  ...
  tag nist: ['AC-3 (11)']
  tag control: 'AC-3 (11)'
  tag objective: 'AC-03(11)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_3_11: true
  tag odp: %w{ac_03_11_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-03(11)' do
