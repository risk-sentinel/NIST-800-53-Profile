control 'AC-06(05)' do
  impact 0.5
  title "privileged accounts on the system are restricted to #{input('ac_06_05_odp')}."
  desc <<~DESC
    Restrict privileged accounts on the system to #{input('ac_06_05_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      privileged accounts on the system are restricted to #{input('ac_06_05_odp')}.
  ...
  tag nist: ['AC-6 (5)']
  tag control: 'AC-6 (5)'
  tag objective: 'AC-06(05)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ac_6_5: true
  tag odp: %w{ac_06_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-06(05)' do
