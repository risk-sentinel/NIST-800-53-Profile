control 'AC-07(03)' do
  impact 0.5
  title "unsuccessful biometric logon attempts are limited to #{input('ac_07_03_odp')}."
  desc <<~DESC
    Limit the number of unsuccessful biometric logon attempts to #{input('ac_07_03_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      unsuccessful biometric logon attempts are limited to #{input('ac_07_03_odp')}.
  ...
  tag nist: ['AC-7 (3)']
  tag control: 'AC-7 (3)'
  tag objective: 'AC-07(03)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_7_3: true
  tag odp: %w{ac_07_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-07(03)' do
