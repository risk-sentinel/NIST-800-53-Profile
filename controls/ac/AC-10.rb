control 'AC-10' do
  impact 0.5
  title "the number of concurrent sessions for each #{input('ac_10_odp_01')} is limited to #{input('ac_10_odp_02')}."
  desc <<~DESC
    Limit the number of concurrent sessions for each #{input('ac_10_odp_01')} to #{input('ac_10_odp_02')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the number of concurrent sessions for each #{input('ac_10_odp_01')} is limited to #{input('ac_10_odp_02')}.
  ...
  tag control_ac_10: true
  tag odp: %w{ac_10_odp_01 ac_10_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective AC-10' do
