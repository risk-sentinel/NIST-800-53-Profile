control 'AC-21(02)' do
  impact 0.5
  title "information search and retrieval services that enforce #{input('ac_21_02_odp')} are implemented."
  desc <<~DESC
    Implement information search and retrieval services that enforce #{input('ac_21_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      information search and retrieval services that enforce #{input('ac_21_02_odp')} are implemented.
  ...
  tag nist: ['AC-21 (2)']
  tag control: 'AC-21 (2)'
  tag objective: 'AC-21(02)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_21_2: true
  tag odp: %w{ac_21_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-21(02)' do
