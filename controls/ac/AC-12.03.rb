control 'AC-12(03)' do
  impact 0.5
  title "an explicit message to users is displayed indicating that the session will end in #{input('ac_12_03_odp')}."
  desc <<~DESC
    Display an explicit message to users indicating that the session will end in #{input('ac_12_03_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      an explicit message to users is displayed indicating that the session will end in #{input('ac_12_03_odp')}.
  ...
  tag nist: ['AC-12 (3)']
  tag control: 'AC-12 (3)'
  tag objective: 'AC-12(03)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_12_3: true
  tag odp: %w{ac_12_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-12(03)' do
