control 'AC-04(06)' do
  impact 0.5
  title "information flow control enforcement is based on #{input('ac_04_06_odp')}."
  desc <<~DESC
    Enforce information flow control based on #{input('ac_04_06_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      information flow control enforcement is based on #{input('ac_04_06_odp')}.
  ...
  tag nist: ['AC-4 (6)']
  tag control: 'AC-4 (6)'
  tag objective: 'AC-04(06)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_4_6: true
  tag odp: %w{ac_04_06_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-04(06)' do
