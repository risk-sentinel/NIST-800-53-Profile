control 'AC-23' do
  impact 0.5
  title "#{input('ac_23_odp_01')} are employed for #{input('ac_23_odp_02')} to detect and protect against unauthorized data mining."
  desc <<~DESC
    Employ #{input('ac_23_odp_01')} for #{input('ac_23_odp_02')} to detect and protect against unauthorized data mining.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ac_23_odp_01')} are employed for #{input('ac_23_odp_02')} to detect and protect against unauthorized data mining.
  ...
  tag baseline: %w{}
  tag control_ac_23: true
  tag odp: %w{ac_23_odp_01 ac_23_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective AC-23' do
