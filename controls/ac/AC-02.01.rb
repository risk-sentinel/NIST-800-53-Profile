control 'AC-02(01)' do
  impact 0.5
  title "the management of system accounts is supported using #{input('ac_02_01_odp')}."
  desc <<~DESC
    Support the management of system accounts using #{input('ac_02_01_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the management of system accounts is supported using #{input('ac_02_01_odp')}.
  ...
  tag nist: ['AC-2 (1)']
  tag control: 'AC-2 (1)'
  tag objective: 'AC-02(01)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ac_2_1: true
  tag odp: %w{ac_02_01_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-02(01)' do
