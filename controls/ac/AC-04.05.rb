control 'AC-04(05)' do
  impact 0.5
  title "#{input('ac_04_05_odp')} are enforced on embedding data types within other data types."
  desc <<~DESC
    Enforce #{input('ac_04_05_odp')} on embedding data types within other data types.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ac_04_05_odp')} are enforced on embedding data types within other data types.
  ...
  tag nist: ['AC-4 (5)']
  tag control: 'AC-4 (5)'
  tag objective: 'AC-04(05)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_4_5: true
  tag odp: %w{ac_04_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-04(05)' do
