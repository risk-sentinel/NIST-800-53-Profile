control 'AC-02(05)' do
  impact 0.5
  title "users are required to log out when #{input('ac_02_05_odp')}."
  desc <<~DESC
    Require that users log out when #{input('ac_02_05_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      users are required to log out when #{input('ac_02_05_odp')}.
  ...
  tag nist: ['AC-2 (5)']
  tag control: 'AC-2 (5)'
  tag objective: 'AC-02(05)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ac_2_5: true
  tag odp: %w{ac_02_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-02(05)' do
