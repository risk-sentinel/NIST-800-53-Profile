control 'AC-06(08)' do
  impact 0.5
  title "#{input('ac_06_08_odp')} is prevented from executing at higher privilege levels than users executing the software."
  desc <<~DESC
    Prevent the following software from executing at higher privilege levels than users executing the software: #{input('ac_06_08_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ac_06_08_odp')} is prevented from executing at higher privilege levels than users executing the software.
  ...
  tag nist: ['AC-6 (8)']
  tag control: 'AC-6 (8)'
  tag objective: 'AC-06(08)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_6_8: true
  tag odp: %w{ac_06_08_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-06(08)' do
