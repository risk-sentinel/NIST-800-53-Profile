control 'AC-16(09)[01]' do
  impact 0.5
  title "security attributes associated with information are changed only via regrading mechanisms validated using #{input('ac_16_09_odp_01')};"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      security attributes associated with information are changed only via regrading mechanisms validated using #{input('ac_16_09_odp_01')};
  ...
  tag nist: ['AC-16 (9)']
  tag control: 'AC-16 (9)'
  tag objective: 'AC-16(09)[01]'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_16_9: true
  tag odp: %w{ac_16_09_odp_01 ac_16_9_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective AC-16(09)[01]' do
