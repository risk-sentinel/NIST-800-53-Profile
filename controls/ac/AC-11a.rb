control 'AC-11a.' do
  impact 0.5
  title "further access to the system is prevented by #{input('ac_11_odp_01')};"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      further access to the system is prevented by #{input('ac_11_odp_01')};
  ...
  tag nist: ['AC-11']
  tag control: 'AC-11'
  tag objective: 'AC-11a.'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_ac_11: true
  tag odp: %w{ac_11_odp_01}

  describe 'NIST SP 800-53A Rev 5 objective AC-11a.' do
