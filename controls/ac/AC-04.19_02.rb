control 'AC-04(19)[02]' do
  impact 0.5
  title "when transferring information between different security domains, #{input('ac_04_19_odp_02')} are implemented on metadata."
  desc <<~DESC
    When transferring information between different security domains, implement #{input('ac_4_19_prm_1')} on metadata.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      when transferring information between different security domains, #{input('ac_04_19_odp_02')} are implemented on metadata.
  ...
  tag nist: ['AC-4 (19)']
  tag control: 'AC-4 (19)'
  tag objective: 'AC-04(19)[02]'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_4_19: true
  tag odp: %w{ac_04_19_odp_02 ac_4_19_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective AC-04(19)[02]' do
