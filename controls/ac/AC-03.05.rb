control 'AC-03(05)' do
  impact 0.5
  title "access to #{input('ac_03_05_odp')} is prevented except during secure, non-operable system states."
  desc <<~DESC
    Prevent access to #{input('ac_03_05_odp')} except during secure, non-operable system states.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      access to #{input('ac_03_05_odp')} is prevented except during secure, non-operable system states.
  ...
  tag nist: ['AC-3 (5)']
  tag control: 'AC-3 (5)'
  tag objective: 'AC-03(05)'
  tag rev: 'Rev_5'
  tag family: 'Access Control'
  tag baseline: %w{}
  tag control_ac_3_5: true
  tag odp: %w{ac_03_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective AC-03(05)' do
