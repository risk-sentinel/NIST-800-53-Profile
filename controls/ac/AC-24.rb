control 'AC-24' do
  impact 0.5
  title "#{input('ac_24_odp_01')} are taken to ensure that #{input('ac_24_odp_02')} are applied to each access request prior to access enforcement."
  desc <<~DESC
    #{input('ac_24_odp_01')} to ensure #{input('ac_24_odp_02')} are applied to each access request prior to access enforcement.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('ac_24_odp_01')} are taken to ensure that #{input('ac_24_odp_02')} are applied to each access request prior to access enforcement.
  ...
  tag baseline: %w{}
  tag control_ac_24: true
  tag odp: %w{ac_24_odp_01 ac_24_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective AC-24' do
