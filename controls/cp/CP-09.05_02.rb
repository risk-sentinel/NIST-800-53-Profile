control 'CP-09(05)[02]' do
  impact 0.5
  title "system backup information is transferred to the alternate storage site #{input('cp_09_05_odp_02')}."
  desc <<~DESC
    Transfer system backup information to the alternate storage site #{input('cp_9_5_prm_1')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      system backup information is transferred to the alternate storage site #{input('cp_09_05_odp_02')}.
  ...
  tag nist: ['CP-9 (5)']
  tag control: 'CP-9 (5)'
  tag objective: 'CP-09(05)[02]'
  tag rev: 'Rev_5'
  tag family: 'Contingency Planning'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_cp_9_5: true
  tag odp: %w{cp_09_05_odp_02 cp_9_5_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective CP-09(05)[02]' do
