control 'CP-10[02]' do
  impact 0.5
  title "a reconstitution of the system to a known state is provided within #{input('cp_10_odp_02')} after a disruption, compromise, or failure."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      a reconstitution of the system to a known state is provided within #{input('cp_10_odp_02')} after a disruption, compromise, or failure.
  ...
  tag control_cp_10: true
  tag odp: %w{cp_10_odp_02 cp_10_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective CP-10[02]' do
