control 'CP-11' do
  impact 0.5
  title "the capability to employ #{input('cp_11_odp')} are provided in support of maintaining continuity of operations."
  desc <<~DESC
    Provide the capability to employ #{input('cp_11_odp')} in support of maintaining continuity of operations.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      the capability to employ #{input('cp_11_odp')} are provided in support of maintaining continuity of operations.
  ...
  tag baseline: %w{}
  tag control_cp_11: true
  tag odp: %w{cp_11_odp}

  describe 'NIST SP 800-53A Rev 5 objective CP-11' do
