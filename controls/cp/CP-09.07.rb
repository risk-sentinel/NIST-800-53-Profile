control 'CP-09(07)' do
  impact 0.5
  title "dual authorization for the deletion or destruction of #{input('cp_09_07_odp')} is enforced."
  desc <<~DESC
    Enforce dual authorization for the deletion or destruction of #{input('cp_09_07_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      dual authorization for the deletion or destruction of #{input('cp_09_07_odp')} is enforced.
  ...
  tag nist: ['CP-9 (7)']
  tag control: 'CP-9 (7)'
  tag objective: 'CP-09(07)'
  tag rev: 'Rev_5'
  tag family: 'Contingency Planning'
  tag baseline: %w{}
  tag control_cp_9_7: true
  tag odp: %w{cp_09_07_odp}

  describe 'NIST SP 800-53A Rev 5 objective CP-09(07)' do
