control 'CP-13' do
  impact 0.5
  title "#{input('cp_13_odp_01')} are employed for satisfying #{input('cp_13_odp_02')} when the primary means of implementing the security function is unavailable or compromised."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('cp_13_odp_01')} are employed for satisfying #{input('cp_13_odp_02')} when the primary means of implementing the security function is unavailable or compromised.
  ...
  tag baseline: %w{}
  tag control_cp_13: true
  tag odp: %w{cp_13_odp_01 cp_13_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective CP-13' do
