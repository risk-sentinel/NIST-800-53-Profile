control 'CP-08(05)' do
  impact 0.5
  title "alternate telecommunications services are tested #{input('cp_08_05_odp')}."
  desc <<~DESC
    Test alternate telecommunication services #{input('cp_08_05_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      alternate telecommunications services are tested #{input('cp_08_05_odp')}.
  ...
  tag nist: ['CP-8 (5)']
  tag control: 'CP-8 (5)'
  tag objective: 'CP-08(05)'
  tag rev: 'Rev_5'
  tag family: 'Contingency Planning'
  tag baseline: %w{}
  tag control_cp_8_5: true
  tag odp: %w{cp_08_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective CP-08(05)' do
