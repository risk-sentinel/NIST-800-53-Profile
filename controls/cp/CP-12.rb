control 'CP-12' do
  impact 0.5
  title "a safe mode of operation is entered with #{input('cp_12_odp_01')} when #{input('cp_12_odp_02')} are detected."
  desc <<~DESC
    When #{input('cp_12_odp_02')} are detected, enter a safe mode of operation with #{input('cp_12_odp_01')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      a safe mode of operation is entered with #{input('cp_12_odp_01')} when #{input('cp_12_odp_02')} are detected.
  ...
  tag baseline: %w{}
  tag control_cp_12: true
  tag odp: %w{cp_12_odp_01 cp_12_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective CP-12' do
