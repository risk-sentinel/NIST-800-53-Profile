control 'SR-05[02]' do
  impact 0.5
  title "#{input('sr_05_odp')} are employed to identify supply chain risks;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sr_05_odp')} are employed to identify supply chain risks;
  ...
  describe 'NIST SP 800-53A Rev 5 objective SR-05[02]' do
