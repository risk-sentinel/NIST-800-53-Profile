control 'SR-03(02)' do
  impact 0.5
  title "#{input('sr_03_02_odp')} are employed to limit harm from potential adversaries identifying and targeting the organizational supply chain."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('sr_03_02_odp')} are employed to limit harm from potential adversaries identifying and targeting the organizational supply chain.
  ...
  tag nist: ['SR-3 (2)']
  tag control: 'SR-3 (2)'
  tag objective: 'SR-03(02)'
  tag rev: 'Rev_5'
  tag family: 'Supply Chain Risk Management'
  tag baseline: %w{}
  tag control_sr_3_2: true
  tag odp: %w{sr_03_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective SR-03(02)' do
