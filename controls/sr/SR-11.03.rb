control 'SR-11(03)' do
  impact 0.5
  title "scanning for counterfeit system components is conducted #{input('sr_11_03_odp')}."
  desc <<~DESC
    Scan for counterfeit system components #{input('sr_11_03_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      scanning for counterfeit system components is conducted #{input('sr_11_03_odp')}.
  ...
  tag nist: ['SR-11 (3)']
  tag control: 'SR-11 (3)'
  tag objective: 'SR-11(03)'
  tag rev: 'Rev_5'
  tag family: 'Supply Chain Risk Management'
  tag baseline: %w{}
  tag control_sr_11_3: true
  tag odp: %w{sr_11_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective SR-11(03)' do
