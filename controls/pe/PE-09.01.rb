control 'PE-09(01)' do
  impact 0.5
  title "redundant power cabling paths that are physically separated by #{input('pe_09_01_odp')} are employed."
  desc <<~DESC
    Employ redundant power cabling paths that are physically separated by #{input('pe_09_01_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      redundant power cabling paths that are physically separated by #{input('pe_09_01_odp')} are employed.
