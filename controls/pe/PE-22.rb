control 'PE-22' do
  impact 0.5
  title "#{input('pe_22_odp')} are marked indicating the impact level or classification level of the information permitted to be processed, stored, or transmitted by the hardware component."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('pe_22_odp')} are marked indicating the impact level or classification level of the information permitted to be processed, stored, or transmitted by the hardware component.
  ...
  tag baseline: %w{}
  tag control_pe_22: true
  tag odp: %w{pe_22_odp}

  describe 'NIST SP 800-53A Rev 5 objective PE-22' do
