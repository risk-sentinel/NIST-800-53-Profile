control 'PE-03(04)' do
  impact 0.5
  title "lockable physical casings are used to protect #{input('pe_03_04_odp')} from unauthorized access."
  desc <<~DESC
    Use lockable physical casings to protect #{input('pe_03_04_odp')} from unauthorized physical access.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      lockable physical casings are used to protect #{input('pe_03_04_odp')} from unauthorized access.
  ...
  tag nist: ['PE-3 (4)']
  tag control: 'PE-3 (4)'
  tag objective: 'PE-03(04)'
  tag rev: 'Rev_5'
  tag family: 'Physical and Environmental Protection'
  tag baseline: %w{}
  tag control_pe_3_4: true
  tag odp: %w{pe_03_04_odp}

  describe 'NIST SP 800-53A Rev 5 objective PE-03(04)' do
