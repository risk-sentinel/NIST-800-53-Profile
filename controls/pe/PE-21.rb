control 'PE-21' do
  impact 0.5
  title "#{input('pe_21_odp_01')} are employed against electromagnetic pulse damage for #{input('pe_21_odp_02')}."
  desc <<~DESC
    Employ #{input('pe_21_odp_01')} against electromagnetic pulse damage for #{input('pe_21_odp_02')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('pe_21_odp_01')} are employed against electromagnetic pulse damage for #{input('pe_21_odp_02')}.
  ...
  tag baseline: %w{}
  tag control_pe_21: true
  tag odp: %w{pe_21_odp_01 pe_21_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective PE-21' do
