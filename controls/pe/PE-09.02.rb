control 'PE-09(02)' do
  impact 0.5
  title "automatic voltage controls for #{input('pe_09_02_odp')} are employed."
  desc <<~DESC
    Employ automatic voltage controls for #{input('pe_09_02_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      automatic voltage controls for #{input('pe_09_02_odp')} are employed.
  ...
  tag nist: ['PE-9 (2)']
  tag control: 'PE-9 (2)'
  tag objective: 'PE-09(02)'
  tag rev: 'Rev_5'
  tag family: 'Physical and Environmental Protection'
  tag baseline: %w{}
  tag control_pe_9_2: true
  tag odp: %w{pe_09_02_odp}

  describe 'NIST SP 800-53A Rev 5 objective PE-09(02)' do
