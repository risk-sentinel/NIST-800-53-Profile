control 'PE-05' do
  impact 0.5
  title "physical access to output from #{input('pe_05_odp')} is controlled to prevent unauthorized individuals from obtaining the output."
  desc <<~DESC
    Control physical access to output from #{input('pe_05_odp')} to prevent unauthorized individuals from obtaining the output.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      physical access to output from #{input('pe_05_odp')} is controlled to prevent unauthorized individuals from obtaining the output.
  ...
  tag control_pe_5: true
  tag odp: %w{pe_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective PE-05' do
