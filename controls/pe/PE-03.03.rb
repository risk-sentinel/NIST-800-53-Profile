control 'PE-03(03)' do
  impact 0.5
  title "guards are employed to control #{input('pe_03_03_odp')} to the facility where the system resides 24 hours per day, 7 days per week."
  desc <<~DESC
    Employ guards to control #{input('pe_03_03_odp')} to the facility where the system resides 24 hours per day, 7 days per week.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      guards are employed to control #{input('pe_03_03_odp')} to the facility where the system resides 24 hours per day, 7 days per week.
  ...
  tag nist: ['PE-3 (3)']
  tag control: 'PE-3 (3)'
  tag objective: 'PE-03(03)'
  tag rev: 'Rev_5'
  tag family: 'Physical and Environmental Protection'
  tag baseline: %w{}
  tag control_pe_3_3: true
  tag odp: %w{pe_03_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective PE-03(03)' do
