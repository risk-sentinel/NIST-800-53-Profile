control 'MA-07' do
  impact 0.5
  title "field maintenance on #{input('ma_07_odp_01')} are restricted or prohibited to #{input('ma_07_odp_02')}."
  desc <<~DESC
    Restrict or prohibit field maintenance on #{input('ma_07_odp_01')} to #{input('ma_07_odp_02')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      field maintenance on #{input('ma_07_odp_01')} are restricted or prohibited to #{input('ma_07_odp_02')}.
  ...
  tag baseline: %w{}
  tag control_ma_7: true
  tag odp: %w{ma_07_odp_01 ma_07_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective MA-07' do
