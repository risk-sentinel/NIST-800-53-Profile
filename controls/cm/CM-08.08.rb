control 'CM-08(08)' do
  impact 0.5
  title "#{input('cm_08_08_odp')} are used to support the tracking of system components by geographic location."
  desc <<~DESC
    Support the tracking of system components by geographic location using #{input('cm_08_08_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('cm_08_08_odp')} are used to support the tracking of system components by geographic location.
  ...
  tag nist: ['CM-8 (8)']
  tag control: 'CM-8 (8)'
  tag objective: 'CM-08(08)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{}
  tag control_cm_8_8: true
  tag odp: %w{cm_08_08_odp}

  describe 'NIST SP 800-53A Rev 5 objective CM-08(08)' do
