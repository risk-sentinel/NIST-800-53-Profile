control 'CM-12c.[01]' do
  impact 0.5
  title "changes to the location (i.e., system or system components) where #{input('cm_12_odp')} is processed are documented;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      changes to the location (i.e., system or system components) where #{input('cm_12_odp')} is processed are documented;
  ...
  describe 'NIST SP 800-53A Rev 5 objective CM-12c.[01]' do
