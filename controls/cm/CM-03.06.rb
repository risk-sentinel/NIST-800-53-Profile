control 'CM-03(06)' do
  impact 0.5
  title "cryptographic mechanisms used to provide #{input('cm_03_06_odp')} are under configuration management."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      cryptographic mechanisms used to provide #{input('cm_03_06_odp')} are under configuration management.
  ...
  tag nist: ['CM-3 (6)']
  tag control: 'CM-3 (6)'
  tag objective: 'CM-03(06)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_cm_3_6: true
  tag odp: %w{cm_03_06_odp}

  describe 'NIST SP 800-53A Rev 5 objective CM-03(06)' do
