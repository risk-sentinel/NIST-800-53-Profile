control 'CM-03(08)' do
  impact 0.5
  title "changes to the configuration of the system are prevented or restricted under #{input('cm_03_08_odp')}."
  desc <<~DESC
    Prevent or restrict changes to the configuration of the system under the following circumstances: #{input('cm_03_08_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      changes to the configuration of the system are prevented or restricted under #{input('cm_03_08_odp')}.
  ...
  tag nist: ['CM-3 (8)']
  tag control: 'CM-3 (8)'
  tag objective: 'CM-03(08)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{}
  tag control_cm_3_8: true
  tag odp: %w{cm_03_08_odp}

  describe 'NIST SP 800-53A Rev 5 objective CM-03(08)' do
