control 'CM-06(02)' do
  impact 0.5
  title "#{input('cm_06_02_odp_01')} are taken in response to unauthorized changes to #{input('cm_06_02_odp_02')}."
  desc <<~DESC
    Take the following actions in response to unauthorized changes to #{input('cm_06_02_odp_02')}: #{input('cm_06_02_odp_01')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('cm_06_02_odp_01')} are taken in response to unauthorized changes to #{input('cm_06_02_odp_02')}.
  ...
  tag nist: ['CM-6 (2)']
  tag control: 'CM-6 (2)'
  tag objective: 'CM-06(02)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{HIGH}
  tag baseline_high: true
  tag control_cm_6_2: true
  tag odp: %w{cm_06_02_odp_01 cm_06_02_odp_02}

  describe 'NIST SP 800-53A Rev 5 objective CM-06(02)' do
