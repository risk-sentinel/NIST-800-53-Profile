control 'CM-05(04)[01]' do
  impact 0.5
  title "dual authorization for implementing changes to #{input('cm_05_04_odp_01')} is enforced;"
  desc <<~DESC
    Enforce dual authorization for implementing changes to #{input('cm_5_4_prm_1')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      dual authorization for implementing changes to #{input('cm_05_04_odp_01')} is enforced;
  ...
  tag nist: ['CM-5 (4)']
  tag control: 'CM-5 (4)'
  tag objective: 'CM-05(04)[01]'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{}
  tag control_cm_5_4: true
  tag odp: %w{cm_05_04_odp_01 cm_5_4_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective CM-05(04)[01]' do
