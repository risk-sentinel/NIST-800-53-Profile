control 'CM-02(03)' do
  impact 0.5
  title "#{input('cm_02_03_odp')} of previous baseline configuration version(s) of the system is/are retained to support rollback."
  desc <<~DESC
    Retain #{input('cm_02_03_odp')} of previous versions of baseline configurations of the system to support rollback.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('cm_02_03_odp')} of previous baseline configuration version(s) of the system is/are retained to support rollback.
  ...
  tag nist: ['CM-2 (3)']
  tag control: 'CM-2 (3)'
  tag objective: 'CM-02(03)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{MODERATE HIGH}
  tag baseline_moderate: true
  tag baseline_high: true
  tag control_cm_2_3: true
  tag odp: %w{cm_02_03_odp}

  describe 'NIST SP 800-53A Rev 5 objective CM-02(03)' do
