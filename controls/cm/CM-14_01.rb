control 'CM-14[01]' do
  impact 0.5
  title "the installation of #{input('cm_14_odp_01')} is prevented unless it is verified that the software has been digitally signed using a certificate recognized and approved by the organization;"
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      the installation of #{input('cm_14_odp_01')} is prevented unless it is verified that the software has been digitally signed using a certificate recognized and approved by the organization;
  ...
  tag nist: ['CM-14']
  tag control: 'CM-14'
  tag objective: 'CM-14[01]'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{}
  tag control_cm_14: true
  tag odp: %w{cm_14_odp_01 cm_14_prm_1}

  describe 'NIST SP 800-53A Rev 5 objective CM-14[01]' do
