control 'CM-03(05)' do
  impact 0.5
  title "#{input('cm_03_05_odp')} are automatically implemented if baseline configurations are changed in an unauthorized manner."
  desc <<~DESC
  ...
  desc 'check', <<~CHECK
    Determine if:
      #{input('cm_03_05_odp')} are automatically implemented if baseline configurations are changed in an unauthorized manner.
  ...
  tag nist: ['CM-3 (5)']
  tag control: 'CM-3 (5)'
  tag objective: 'CM-03(05)'
  tag rev: 'Rev_5'
  tag family: 'Configuration Management'
  tag baseline: %w{}
  tag control_cm_3_5: true
  tag odp: %w{cm_03_05_odp}

  describe 'NIST SP 800-53A Rev 5 objective CM-03(05)' do
