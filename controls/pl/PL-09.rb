control 'PL-09' do
  impact 0.5
  title "#{input('pl_09_odp')} are centrally managed."
  desc <<~DESC
    Centrally manage #{input('pl_09_odp')}.
  DESC
  desc 'check', <<~CHECK
    Determine if:
      #{input('pl_09_odp')} are centrally managed.
  ...
  tag control_pl_9: true
  tag odp: %w{pl_09_odp}

  describe 'NIST SP 800-53A Rev 5 objective PL-09' do
